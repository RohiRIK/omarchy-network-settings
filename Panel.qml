...
Panel {
  ...
  property var ipProfiles: []
  property string ipOperation: ""
  function ipRequest(operation, args) {
    if (ipProc.running) return
    ipOperation = operation
    ipMessage = ""
    ipProc.command = ["python3", Qt.resolvedUrl("settings.py").toString().replace("file://", ""), operation].concat(args || [])
    ipProc.running = true
  }
  function ipLoad(data) {
    ipUuid = data.uuid
    ipMethod = data.method
    ipAddress.text = data.addresses
    ipGateway.text = data.gateway
    ipDns.text = data.dns
  }
  Process {
    id: ipProc
    stdout: StdioCollector {
      onStreamFinished: {
        try {
          ...
          if (data.error) { root.ipMessage = data.error; return }
          if (root.ipOperation === "list") {
            root.ipProfiles = data.profiles
            root.ipUuid = ""
            if (data.selected) root.ipLoad(data.selected)
            root.ipMessage = data.profiles.length ? "" : "Connect to Wi-Fi first, then return here."
          } else if (root.ipOperation === "read" || root.ipOperation === "current") {
            root.ipLoad(data)
            root.ipMessage = data.message || ""
          } else root.ipMessage = data.message
        } catch (e) { root.ipMessage = "Could not read network settings." }
      }
    }
  }
  ...
  IpcHandler {
    ...
    function wifi() { root.ipEditing = false; root.open() }
    function settings() { root.open(); root.ipEditing = true; root.ipRequest("list", []) }
  ...
  KeyboardPanel {
    ...
    contentHeight: panel.fittedContentHeight(root.ipEditing ? ipForm.implicitHeight : column.implicitHeight)
    ...
    PanelKeyCatcher {
      ...
      blocked: root.ipEditing || root.passwordSsid !== ""
      ...
      onTextKey: function(t) {
        if (t === "s" || t === "S") { root.ipEditing = true; root.ipRequest("list", []) }
    ...
    Column {
      id: ipForm
      visible: root.ipEditing
      anchors.left: parent.left
      anchors.right: parent.right
      anchors.top: parent.top
      spacing: Style.space(12)
      Keys.onEscapePressed: { root.ipEditing = false; keyCatcher.forceActiveFocus() }
      Ui.Button {
        text: "‹  Network"
        focusable: true
        onClicked: { root.ipEditing = false; keyCatcher.forceActiveFocus() }
      }
      Text {
        text: "IP settings"
      ...
      Ui.Dropdown {
        width: parent.width
        label: "CONNECTION"
        options: root.ipProfiles
        value: root.ipUuid
        enabled: !ipProc.running
        onChanged: function(value) { root.ipRequest("read", [value]) }
      }
      Ui.Dropdown {
        width: parent.width
        label: "IP ADDRESS MODE"
        options: [{value: "auto", label: "Automatic (DHCP)"}, {value: "manual", label: "Manual (choose an IP)"}]
        value: root.ipMethod
        enabled: root.ipUuid !== "" && !ipProc.running
        onChanged: function(value) { root.ipMethod = value }
      }
      Ui.Button {
        width: parent.width
        text: "Use current IP as fixed"
        focusable: true
        bordered: true
        enabled: root.ipUuid !== "" && !ipProc.running
        onClicked: root.ipRequest("current", [root.ipUuid])
      }
      Text {
        text: "IP ADDRESS / PREFIX"
      ...
      Ui.TextField {
        ...
        visible: root.ipMethod === "manual"
        enabled: !ipProc.running
        placeholderText: "192.168.1.1 (optional)"
      }
      Text {
        text: "DNS SERVERS"
        color: root.bar.foreground
      ...
      Ui.TextField {
        ...
        width: parent.width
        enabled: root.ipUuid !== "" && !ipProc.running
        placeholderText: "Automatic, or 1.1.1.1, 8.8.8.8"
      }
      Text {
        width: parent.width
        text: root.ipMessage || "Save the profile, then reconnect to use the new settings."
      ...
      Ui.Button {
        text: ipProc.running ? "Working…" : "Save settings"
        focusable: true
        bordered: true
        enabled: root.ipUuid !== "" && !ipProc.running && (root.ipMethod === "auto" || root.ipMethod === "manual")
        onClicked: root.ipRequest("save", [root.ipUuid, root.ipMethod, ipAddress.text, ipGateway.text, ipDns.text])
      }
    }
